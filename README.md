...
| `b25`     | bin  | Thin CLI front-end (`b25`), API-compatible with the classic C `b25` tool. |
...
```text
b25 [options] src.m2t dst.m2t [more pairs ...]
b25 [options] -      -            # read TS from stdin, write to stdout
...
```
dvbr tune ... | b25 -v 0 - -   →   descrambled TS
```

## Build

```bash
cargo build --release          # produces target/release/b25
```
